...
blocks_num = 10
sortedRCR_VL = table.dropna().sort_values(by=['ROLL_CROSS_RATE'])[['ROLL_CROSS_RATE', 'VAR_L']]
sortedRCR = sortedRCR_VL['ROLL_CROSS_RATE'].values
sortedVL = sortedRCR_VL['VAR_L'].values
...
ax1.set_xlabel(u'滚转操作逆转率', fontsize='x-large', fontproperties=myfont)
ax1.set_ylabel(u'航向道偏差的方差', fontsize='x-large', fontproperties=myfont)
...
for i in range(blocks_num):
    if mask[i]:
        ax2.text(x[i] + 0.1, VLHistMeans[i]+0.0002, str(VLHistMeans[i]), fontsize='small')
ax2.set_xticks(np.arange(blocks_num+1))
ax2.set_xticklabels(edges, rotation=0, size='small')
ax2.set_xlabel(u'滚转操作逆转率', fontsize='x-large', fontproperties=myfont)
ax2.set_ylabel(u'航向道偏差的方差', fontsize='x-large', fontproperties=myfont)
